...
const { getReplyFromOpenAI, parseReminderWithOpenAI } = require('../utils/openai'); // Needs openai.js in utils
const { sendWhatsAppMessage } = require('../utils/whatsapp'); // Needs whatsapp.js in utils
const { getDb } = require('../utils/database'); // <--- السطر ده لازم يكون موجود والمسار صح: database.js في utils
const { addReminder } = require('../scheduler/reminderQueue'); // Needs reminderQueue.js in scheduler
                ...
                if (msg_body && from) {
                    ...
                    if (!reminderProcessed) {
                        if (lowerMsgBody.includes("الساعة كام") || lowerMsgBody.includes("الوقت ايه") || lowerMsgBody === "الوقت") {
                            ...
                            await sendWhatsAppMessage(from, replyMsg); // This will also log the bot reply via the modified function
                    ...
                    if (!reminderProcessed && !handledSpecifically) {
                        ...
                        if (aiReply) {
                            await sendWhatsAppMessage(from, aiReply); // This will also log the bot reply
                        } else {
